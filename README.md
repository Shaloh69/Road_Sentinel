...
├── client/web/                # Next.js dashboard — see DESIGN.md for the design system
...
└── Summarization.md          # Full phase-by-phase revamp record (Phases 0-5)
```

There is no `scripts/` layout, no `models/v1`/`v2`/`production`, and no `train_vehicle_detector.py`/`quick_train.py`/`auto_download_coco.py`/`angled_camera_calibration.py` anywhere in this repo — those were an earlier layout that never matched what's tracked. See `documentation.md §15` for the full history of what moved where.
...
- **MySQL** — see `Summarization.md` Phase 0.5/4 for the current hosting status before assuming any specific instance is live
...
- **`documentation.md`** — ground-truth audit of the whole codebase
- **`Summarization.md`** — full phase-by-phase record of this revamp (Phases 0-5), including what's live-verified vs. hardware-blocked
- **`client/web/DESIGN.md`** — the "Night Watch" design system rationale
- **`training/README.md`**, **`raspi_scripts/README.md`**, **`server/ai-service/README.md`** — component-level docs
- **`TRAINING_GUIDE.md`**, **`PROJECT_STRUCTURE.md`**, **`CAMERA_TEST_GUIDE.md`**, **`START_HERE.md`** — supplementary guides
