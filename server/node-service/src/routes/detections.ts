...
router.get('/', async (req: Request, res: Response) => {
  try {
    const { camera_id, limit = '100', since } = req.query as Record<string, string>;
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const saved: Detection = { ...d, id: result.insertId, timestamp: new Date() };

    // Broadcast to clients subscribed to this camera
    io.to(`camera:${d.camera_id}`).emit('detection', { type: 'detection', data: saved });

    res.status(201).json({ success: true, data: saved } as ApiResponse<Detection>);
  } catch (err) {
