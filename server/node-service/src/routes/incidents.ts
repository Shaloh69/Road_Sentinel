...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch incidents" });
  }
});
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    io.to(`camera:${inc.camera_id}`).emit("incident", {
      type: "incident",
      data: saved,
    });
...
router.put("/:id/status", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Incident ${req.params.id} updated to ${status}`,
    });
  } catch (err) {
    res
      .status(500)
      .json({ success: false, error: "Failed to update incident" });
  }
});
