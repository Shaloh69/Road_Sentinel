...
**What's moving:** `render.env.txt` confirms Node service + Next.js client are currently deployed on Render. Move both off Render entirely onto the user's dedicated PC (RTX 3060 Ti). The AI service's media storage already appears to run locally with a Cloudflare-Tunnel-fronted `STORAGE_BASE_URL` (per `documentation.md §7.1`/§12) — confirm that's still correct post-migration rather than rebuilding it; it may already be most of the way there. **MySQL also moves onto the same PC — but as a fresh install, not a migration**: Aiven is being abandoned outright (its data is presumed unrecoverable, per the user's decision), not dumped and restored.

**Database — drop Aiven entirely, fresh MySQL on `irm-pc`:**
- Aiven is being abandoned outright, not migrated from — its hostname already returns `NXDOMAIN` (per Phase 4's audit finding), and the user has decided not to chase recovering it. **No `mysqldump`/data-recovery step is needed.** Whatever was in Aiven is gone; start clean.
...
- Initialize the schema directly from `migrate.ts` (already confirmed as the authoritative source per Phase 0 item 7's schema-drift fix) against the new empty local database — this also fully resolves that schema-drift item, since there's no old dump to reconcile against anymore.
- Update `server/node-service`'s `DB_HOST`/`PORT`/`USER`/`PASSWORD`/`NAME`/`SSL` to point at the local instance (`DB_HOST=localhost` or `127.0.0.1`, `DB_SSL=false` — no TLS needed for a local loopback connection on the same machine).
- **MySQL should never be exposed publicly at all** — not through the Cloudflare Tunnel, not even through Tailscale. Bind it to localhost only. This is a meaningful security improvement over the Aiven setup, not just a hosting change.
- The `render.env.txt` Aiven credentials (already redacted per Phase 0 item 3) can now be removed from the codebase entirely rather than kept as rotated-but-unused placeholders, since there's no Aiven instance left to reference.
- No further action needed on the Aiven dashboard — the user has decided not to pursue checking whether the service/data still exists there.
